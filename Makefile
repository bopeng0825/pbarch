...
ifeq ($(platform), trimui)
	...
	SOURCES += menu_sdl2.c menu_style.c text_cache.c
	...
	SOURCES := $(filter-out libpicofe/in_sdl.c,$(SOURCES))
	SOURCES += plat_h150102.c plat_h150101_sdl2_input.c
	SOURCES += menu_sdl2.c menu_style.c text_cache.c
	CFLAGS += -DUSE_SDL2 $(shell $(SYSROOT)/usr/bin/sdl2-config --cflags)
	...
	SOURCES += plat_linux.c menu_sdl2.c menu_style.c text_cache.c
	CFLAGS += -DUSE_SDL2 $(shell $(SYSROOT)/usr/bin/sdl2-config --cflags)
	LDFLAGS += -lSDL2_ttf $(shell $(SYSROOT)/usr/bin/sdl2-config --libs)
	LDFLAGS += -fPIE
endif
